...
class Skeleton_in_Context(nn.Module):
    ...
    def generate_mask(self, seq, mask_type):
        '''
        ...
        B, F2, _, _ = seq.shape
        if mask_type == "random":
            # generate mask 训练和测试时mask的策略不同，训练时随机mask，测试时mask后半部分
            if self.training:
                bool_masked_pos = self.mask_center_rand_v1(seq) # B 2F
            else:
                vis = torch.zeros((seq.shape[0], seq.shape[1]//2))
                mask = torch.ones((seq.shape[0], seq.shape[1]//2))
                bool_masked_pos = torch.cat([vis, mask], dim=1).to(torch.bool) # B 2F
        elif mask_type == "mask_half":
    ...
    def forward(self, Prompt, Query, epoch=None):
        ...
        B, F2, J, C = Prompt.shape # B: batch_size, F: number of frames, J: 17 joints, C: 3 dims
        F = F2 // 2

        Prompt_embed = self.encode(Prompt)  # (B,2F,17,512)
        Query_embed = self.encode(Query)  # (B,2F,17,512)

        bool_masked_pos = self.generate_mask(Query_embed, self.mask_type)

        TUP = torch.einsum('btjk,kc->btjc', self.TUP_ST, self.TUP_C)
        TUP = TUP.expand(B, -1, -1, -1)

        m = bool_masked_pos.unsqueeze(-1).unsqueeze(-1).type_as(TUP).reshape(B, 2 * F, 1, 1) # rand_mask_v1  # (B,2F,1,1)

        Query_embed = Query_embed * (1 - m) + TUP * m   # (B,2F,17,512)
        ...
        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            if idx == self.merge_idx:
                x = x.reshape(2 * B,  2 * F, J, self.dim_feat)
                x = (x[:B] + x[B:]) * 0.5
                x = x.reshape(B * 2 * F, J, self.dim_feat)
            x_st = blk_st(x, 2*F)
            x_ts = blk_ts(x, 2*F)
            if self.att_fuse:
                ...
                x = (x_st + x_ts)*0.5
        x = self.norm(x)
        x = x.reshape(B, 2 * F, J, -1)
        x = self.pre_logits(x)
        x = self.head(x)
        target = Query.clone().detach()

        rebuild_part = x[:, F:, :, :].reshape(B, -1, J, self.dim_out)
        target_part = target[:, F:, :, :].reshape(B, -1, J, self.dim_out)
        ...
        return rebuild_part, target_part
